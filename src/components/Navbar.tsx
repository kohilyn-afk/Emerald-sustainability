...
import { ShieldCheck, Menu, X, ArrowUpRight, Sparkles, Calculator, Layers, FileSpreadsheet, User, PhoneCall } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenContact, activeSection }) => {
  ...
  const navLinks = [
    { name: 'AI Diagnostic', href: '#diagnostic', icon: Sparkles, badge: 'AI' },
    { name: 'ROI Calculator', href: '#roi-calculator', icon: Calculator },
    { name: 'Live Suite Prototype', href: '#live-prototype', icon: FileSpreadsheet },
    ...
    <header className="sticky top-0 z-50 bg-[#091710]/92 backdrop-blur-md border-b border-[#1f4230] transition-all">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden sm:flex items-center gap-3">
            <button
              ...
              className="relative inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold gold-gradient-btn transition-all gold-glow"
            >
              <PhoneCall className="w-3.5 h-3.5 text-[#08150e]" />
              <span>Book Consultation</span>
              <ArrowUpRight className="w-3.5 h-3.5 text-[#08150e]" />
            </button>
          </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#0d2217] border-b border-[#1f4230] px-4 pt-2 pb-6 space-y-2">
          ...
          <div className="pt-3 border-t border-[#1f4230]">
            <button
              onClick={() => {
                setMobileMenuOpen(false);
                onOpenContact();
              }}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold gold-gradient-btn"
            >
              <PhoneCall className="w-4 h-4" />
              <span>Book Direct C-Suite Consultation</span>
            </button>
          </div>
        </div>
      )}
